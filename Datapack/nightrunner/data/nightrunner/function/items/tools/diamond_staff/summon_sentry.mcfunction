...
execute as @e[type=block_display,tag=sentry,tag=just_spawned] run function nightrunner:items/tools/diamond_staff/setup_spawned_sentry
function nightrunner:items/tools/diamond_staff/check_mana
execute as @a[limit=1,tag=raycasting,sort=nearest,gamemode=!creative] run scoreboard players add @s Nightrunner_MagicSkillPoints 16
